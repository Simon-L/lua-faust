local custom_open_tag = "{{"
local custom_close_tag = "}}"

-- included from https://github.com/leafo/etlua with some changes
-- - template open/close tags changed to {{ and }}
function etlua()
    local VERSION = "1.3.0"    
    local insert, concat
    do
    ...
    do
      local _class_0
      local _base_0 = {
        open_tag = custom_open_tag ~= nil and custom_open_tag or "{{",
        close_tag = custom_close_tag ~= nil and custom_close_tag or "}}",
        modifiers = "^[=-]",
